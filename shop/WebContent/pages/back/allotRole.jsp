...
<body style="padding:20px 40px 0px 40px;">
	<form class="layui-form" id="mesFrom" method="post" lay-filter="from">
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-inline">
				<input type="text" name="manUser" id="manUser" required
	...
	$(function (){
		var manId = GetQueryString("manId");
		findById(manId);
	});
	
	function findById(manId){
		if(manId!=null || manId!=''){
			var url = "manager/back/findById.action";
			var data = {"manId":manId};
	...
	*/
	function submitFrom(){
		var url = "privilege/back/updateManMod.action";
		var arr = "";
		...
		arr += "";
		var data = {"manId":$("#manId").val(), "roleIdArr":arr};
		layer.load();
	...
	var setting = {
		async : {
			enable : true,//采用异步加载
			url : "privilege/back/showRole.action",
			dataType : "json"
		},
		check : {
			enable : true,//采用异步加载
	...
	function zTreeOnAsyncSuccess(event, treeId, msg) {
		...
		var url = "privilege/back/findByManId.action";
		var data = {"manId": GetQueryString("manId")};
